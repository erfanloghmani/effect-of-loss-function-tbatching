...
#             user_timediff = timestamp_sequence[pidx] - timestamp_sequence[seq[i - 1]] if i else timestamp_sequence[pidx]
            ...
            if item_previous_interaction[pidx] == -1:
                ...
                item_embedding_input = item_embeddings_timeseries[item_previous_interaction[pidx], :].unsqueeze(0)
...
#             item_embeddings[itemid, :] = item_embedding_output.squeeze(0)
        ...
        all_predictions.append(predicted_item_embedding.detach().cpu().numpy())
...
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        my_euclidean_distances_smaller = (my_euclidean_distances < my_true_item_distance).data.cpu().numpy()
        my_true_item_rank = np.sum(my_euclidean_distances_smaller) + 1

        if j < test_start_idx:
            validation_ranks_my.append(my_true_item_rank)
        else:
            test_ranks_my.append(my_true_item_rank)
...
json.dump(validation_ranks_my, open('results/validation_ranks_my_%s_%s_%s.json' % (args.epoch, args.model, args.network), 'w'))
json.dump(test_ranks_my, open('results/test_ranks_my_%s_%s_%s.json' % (args.epoch, args.model, args.network), 'w'))
