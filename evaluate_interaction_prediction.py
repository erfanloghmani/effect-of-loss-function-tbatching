...
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        euclidean_distances_words = nn.PairwiseDistance()(cur_user_last_word_in_cluster.view(-1, 32).repeat(num_items, 1), torch.tensor(item_word_embs.repeat(N_CLUSTERS, 0), dtype=torch.float).cuda()).squeeze(-1).view(-1, num_items, 32).transpose(2, 1)
