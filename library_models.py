...
class JODIE(nn.Module):
    def __init__(self, args, num_features, num_users, num_items):
        ...
        self.prediction_layer = nn.Linear(self.user_static_embedding_size + self.item_static_embedding_size + self.embedding_dim * 2, 2 * (self.item_static_embedding_size + self.embedding_dim))
...
def save_model(model, optimizer, args, epoch, user_embeddings, item_embeddings, train_end_idx, user_embeddings_time_series=None, item_embeddings_time_series=None, path=PATH):
    ...
    filename = os.path.join(directory, "min-multimode-checkpoint.%s.ep%d.tp%.1f.pth.tar" % (args.model, epoch, args.train_proportion))
...
def load_model(model, optimizer, args, epoch, device):
    ...
    filename = PATH + "saved_models/%s/min-multimode-checkpoint.%s.ep%d.tp%.1f.pth.tar" % (args.network, modelname, epoch, args.train_proportion)
